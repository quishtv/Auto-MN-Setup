#!/bin/bash
# QUISH Masternode Setup Script V1 for Ubuntu 18.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'QUISHd' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop QUISHd${NC}"
        QUISH-cli stop
        sleep 30
        if pgrep -x 'QUISHd' > /dev/null; then
            echo -e "${RED}QUISHd daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            sudo pkill -9 QUISHd
            sleep 30
            if pgrep -x 'QUISHd' > /dev/null; then
                echo -e "${RED}Can't stop QUISHd! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${GREEN} ------- QUISH MASTERNODE INSTALLER v1.0.0--------+
 ...
 |        the masternode under a user QUISH.         |::
 |                                                  |::
...
wget https://github.com/quishtv/QUISH/releases/download/1.0.0.0/QUISH-Ubuntu_daemon_18.04.tar.gz
tar -xzvf QUISH-Ubuntu_daemon_18.04.tar.gz
...
cd ~/.QUISH/ && https://github.com/quishtv/QUISH/releases/download/1.0.0.0/bootstrap.zip
...
======================================================================== \a"
echo -e "${GREEN}QUISH_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it 
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}QUISH_mn1${NC} - with your desired masternode name (alias)
...
echo -e "${NC}-------------------------------------------------
NOTE: To edit QUISH.conf, first stop the QUISHd daemon,
then edit the QUISH.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the QUISHd daemon back up:
